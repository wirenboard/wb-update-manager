...
def release_exists(state: SystemState):
    if state.target.endswith("trixie"):
        return True
...
def generate_release_apt_experimental_preferences(
    state: SystemState, filename=WB_RELEASE_APT_EXPERIMENTAL_PREFERENCES_FILENAME
):
    priority = 991
    if state.target.endswith("trixie"):
        priority = 1000
...
def generate_system_config(state):
    ...
    logger.info("Generating priority for experimental packages")
    generate_release_apt_experimental_preferences(
        state, filename=WB_RELEASE_APT_EXPERIMENTAL_PREFERENCES_FILENAME
    )
