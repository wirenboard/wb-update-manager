...
class TestSystemStateReader:
    ...
    def test_read(self, wb_release, sources_list, result):
        assert (
            release.get_current_state(
                os.path.join(DATA_PATH, wb_release), os.path.join(DATA_PATH, sources_list)
            )
            == result
        )
    ...
    def test_error(self, filename):
        with pytest.raises(Exception):
            release.get_current_state(
                os.path.join(DATA_PATH, filename), os.path.join(DATA_PATH, "sources.3.list")
            )
...
class TestTargetStateGenerator:
    def test_impossible_update(self):
        with pytest.raises(release.ImpossibleUpdateError):
            release.get_target_state(
                release.SystemState("some", "thing", "here", True), reset_url=True, prefix="new/prefix"
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (release.SystemState("staging", "all", "", True), "deb http://deb.wirenboard.com/all staging main"),
    ],
)
...
def test_tmp_apt_preferences_generator(state, result):
    with tempfile.NamedTemporaryFile() as f:
        ...
        assert (
            read_file_ignore_comments(f.name)
...
def test_system_update_assume_yes(mocker):
    ...
    release.run_apt.assert_has_calls(
        [mocker.call("update", assume_yes=True), mocker.call("dist-upgrade", assume_yes=True)],
        any_order=False,
    )


@pytest.mark.parametrize("filename,expected", [("sources.1.list", "wb-2108"), ("sources.2.list", "wb-2108")])
...
class TestRoute:
    def patch(
        self,
        mocker,
        system_state=release.SystemState("testing", "wb6/stretch", "", True),
        release_exists=True,
    ):
        # possible actions
    ...
    def test_reset_packages_route(self, mocker):
        ...
        release.update_system.assert_called_once_with(
            state, state, second_stage=True, assume_yes=False, log_filename=None
        )

    def test_reset_packages_conflict(self, mocker):
        self.patch(mocker)
        assert release.RETCODE_EINVAL == release.route(
            args=self.make_args(reset_packages=True, reset_url=True), argv=["test", "-p"]
        )
        assert release.RETCODE_EINVAL == release.route(
            args=self.make_args(reset_packages=True, target_release="new_release"), argv=["test", "-p"]
        )
        assert release.RETCODE_EINVAL == release.route(
            args=self.make_args(reset_packages=True, prefix="new/prefix"), argv=["test", "-p"]
        )
    ...
    def test_new_state_exists(self, mocker):
        ...
        release.update_system.assert_called_once_with(
            new_state, state, second_stage=False, assume_yes=False, log_filename=None
        )
...
class TestUpdate:
    ...
    def test_exceptions(self, mocker, exception, retcode, second_stage):
        ...
        if second_stage:
            release.update_second_stage.assert_called_once_with(
                self.new_state, self.old_state, assume_yes=False
            )
...
class TestUpdateFirstStage(TestUpdateStageBase):
    ...
    def test_confirmed_log_forward(self, mocker, assume_yes):
        ...
        subprocess.run.assert_called_once_with(
            (argv + ["--no-preliminary-update", "--log-filename", log_filename]), check=True
        )
